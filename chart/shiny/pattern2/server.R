...
createPatternLink<-function(data.patternId){
  sprintf('<a href="../pattern2/?patternId=%s"  class="btn btn-primary">Link</a>',data.patternId)
}

createChartLink<-function(data.patternId,data.lineId){
  sprintf('<a href="../chart2/?patternId=%s&lineId=%s"  class="btn btn-primary">Link</a>',data.patternId,data.lineId)
}
...
shinyServer(function(input, output,session) {
  ...
  output$patternView <- renderDataTable({
    ...
    if (length(query)>0){
      res<-dbSendQuery(con,paste("select stockCode,signalTime,price,upDownFlag,lineID from chartPatternSignal2 where patternId='",query$patternId,"' order by signalTime desc",sep=""))
      data.df<-fetch(res,-1)
      ...
      res<-dbSendQuery(con,"select m.id,m.description,count(s.stockCode) as count from patternMaster m left outer join chartPatternSignal2 s on m.id=s.patternId group by m.id")
      data.df<-fetch(res,-1)
