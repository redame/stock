...
make_elliott<-function(ohlc,con,stockCode,date_from,date_to,zigzag,ashi,table,col){
  query<-paste("select * from ptnElliott where stockCode='",stockCode,"' and zigzag=",zigzag," and date1>='",date_from,"' and date9<='",date_to,"' and ashi='",ashi,"' order by date9 asc",sep="")
  cat(file=stderr(),query)
  lines<-fetch(dbSendQuery(con,query),n=-1)
  for(i in 1:length(lines$stockCode)){ 
    line<-lines[i,]
    tl<-data.frame(Date=as.POSIXct(ohlc$Date),Close=rep(NA,length=length(ohlc$Date)))
    tl<-make_trendline(tl,line$date1,line$price1,line$date2,line$price2)
    tl<-make_trendline(tl,line$date2,line$price2,line$date3,line$price3)
    tl<-make_trendline(tl,line$date3,line$price3,line$date4,line$price4)
    tl<-make_trendline(tl,line$date4,line$price4,line$date5,line$price5)
    tl<-make_trendline(tl,line$date5,line$price5,line$date6,line$price6)
    tl<-make_trendline(tl,line$date6,line$price6,line$date7,line$price7)
    tl<-make_trendline(tl,line$date7,line$price7,line$date8,line$price8)
    tl<-make_trendline(tl,line$date8,line$price8,line$date9,line$price9)
    plot(addTA(read.zoo(tl),legend=NULL,on = 1,col=col,type="l"))
  }
}
...
shinyServer(function(input, output, session) {
  ...
  output$returnButton<- renderText({
    ...
    str<-sprintf('<a href="../ptn_elliott_list/?ptn=%s&ashi=%s"  class="btn btn-primary">Back</a>',query$ptn,query$ashi)
  ...
  output$stockChart <- renderPlot({
    ...
    make_elliott(ohlc,con,stockCode,line1_stDate,line1_edDate,zigzagElliott,ashi,"ptnElliott",2)
  ...
  output$ptnList<-renderDataTable({
    ...
    #cat(file=stderr(),dt)
    query<-paste(
"select  'ptnElliott' as ptn,stockCode,zigzag,ashi,date1 as date_from,date9 as date_to from ptnElliott where stockCode='",stockCode,"' and date1>=",dt[1]," and date9<=",dt[2]," and ashi='",ashi,"' and zigzag=",zigzagElliott," union ",
