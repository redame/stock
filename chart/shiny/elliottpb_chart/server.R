...
shinyServer(function(input, output, session) {
  ...
  output$stockChart <- renderPlot({
    ...
    data.lines<-fetch(dbSendQuery(con,paste("select * from elliottPB where stockCode='",stockCode,"' and date1>='",fromDate,"' and date9<='",toDate,"' and ashi='",ashi,"' order by date9 asc",sep="")),-1)
...
#    }
  ...
  output$patternData <- renderDataTable({
    ...
    data.lines<-fetch(dbSendQuery(con,paste("select ashi,pb_term,date_format(date1,'%Y-%m-%d') as date1,date_format(date2,'%Y-%m-%d') as date2,date_format(date3,'%Y-%m-%d') as date3,date_format(date4,'%Y-%m-%d') as date4,date_format(date5,'%Y-%m-%d') as date5,date_format(date6,'%Y-%m-%d') as date6,date_format(date7,'%Y-%m-%d') as date7,date_format(date8,'%Y-%m-%d') as date8,date_format(date9,'%Y-%m-%d') as date9 from elliottPB where stockCode='",stockCode,"' and date1>='",fromDate,"' and date9<='",toDate,"' and ashi='",ashi,"' order by date9 asc",sep="")),-1)
    dbDisconnect(con)
    return(data.lines)
