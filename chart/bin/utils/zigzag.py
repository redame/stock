...
@jit('i8(f8[:],f8,f8)')
def _identify_initial_pivot(open, high, low, close, up_thresh, down_thresh):
    ...
    for t in range(1, len(close)):
        ...
        if x_mim_t / min_x >= up_thresh:
            return VALLEY if min_t == 0 else PEAK

        if x_max_t / max_x <= down_thresh:
    ...
    t_n = len(close)-1
    return VALLEY if x_0 < low[t_n] else PEAK


@jit('i1[:](f8[:],f8,f8)')
def peak_valley_pivots2(open, high, low, close, up_thresh, down_thresh):
    """
    ...
    initial_pivot = _identify_initial_pivot(open, high, low, close, up_thresh, down_thresh)
    ...
    for t in range(1, len(close)):
        ...
        x_min = low[t]
        # r = x / last_pivot_x
        r_min = x_min / last_pivot_x
        r_max = x_max / last_pivot_x
        ...
        if trend == -1:
            if r_min >= up_thresh:
                ...
                last_pivot_x = x_min
                last_pivot_t = t
            elif x_min < last_pivot_x:
                last_pivot_x = x_min
                last_pivot_t = t
        else:
            if r_max <= down_thresh:
                pivots[last_pivot_t] = trend
                trend = -1
                last_pivot_x = x_max
                last_pivot_t = t
            elif x_max > last_pivot_x:
                last_pivot_x = x_max
                last_pivot_t = t
