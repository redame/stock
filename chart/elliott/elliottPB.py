...
class Elliott:
    ...
    def __write_to_db(self,dates,prices):
        print self.stockCode+","+str(self.pb_term)+","+dates[0]+","+dates[8]
        cursor=self.connection2.cursor()
        cursor.execute("replace into fintech.elliottPB values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",(self.stockCode,self.pb_term,dates[0],prices[0],dates[1],prices[1],dates[2],prices[2],dates[3],prices[3],dates[4],prices[4],dates[5],prices[5],dates[6],prices[6],dates[7],prices[7],dates[8],prices[8],self.ashi))
...
class Calcurator:
    ...
    def execute(self):
        cursor=self.connection.cursor()
        cursor.execute("select distinct stockCode from "+self.table+" where date>=%s and date<=%s  order by stockCode ",(self.fromDate,self.toDate))
        stockCodes = cursor.fetchall()
        for stockCode in stockCodes:
            for i in range(5,100,5):
                print stockCode[0]+","+str(i)
                Elliott(stockCode[0],self.fromDate,self.toDate,i,self.ashi,self.table).calc()
