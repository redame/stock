...
calcAll<-function(stock.hist,code){
  calcTech<-function(stock.hist,term){
    tech<-NA
    tryCatch({
      tech<-RSI(stock.hist[,2],n=term,type="EMA")
      return (tech)
    },
    error=function(e){
  ...
  if(length(stock.hist[,2])>0){ 
    ...
    stock.n1<-calcTech(stock.hist,9)
    stock.n2<-calcTech(stock.hist,14)
    stock.tech<-data.frame(date=stock.hist[,1],code=stock.code,ns=stock.n1,nl=stock.n2)
    write.table(stock.tech,file=paste("/home/pi/stock/work/rsi.txt",sep=""),col.names=F,row.names=F,quote=F,sep="\t",na="",append=T)
  }
}
