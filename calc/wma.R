...
calcAll<-function(stock.hist,code){
  ...
  if(length(stock.hist[,2])>0){ 
    ...
    stock.n1<-calcTech(stock.hist,5)
    stock.n2<-calcTech(stock.hist,25)
    stock.n3<-calcTech(stock.hist,75)
    stock.tech<-data.frame(date=stock.hist[,1],code=stock.code,ns=stock.n1,nm=stock.n2,nl=stock.n3)
    write.table(stock.tech,file=paste("/home/pi/stock/work/wma.txt",sep=""),col.names=F,row.names=F,quote=F,sep="\t",na="",append=T)
  }
}
